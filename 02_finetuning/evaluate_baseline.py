...
def _dbt_parse(dbt_dag: str, dbt_cmd: str) -> bool:
    ...
    with tempfile.TemporaryDirectory(prefix='dbt_eval_') as tmpdir:
        ...
        proc = subprocess.run(
            [dbt_cmd, 'parse', '--profiles-dir', str(tmp), '--project-dir', str(tmp),
             '--no-use-colors', '--no-partial-parse'],
...
def _has_marts(dag: str) -> bool:
    return bool(re.search(r'<file path="models/marts/core/(?:fct|dim|rpt)_[^"]+\.sql">', dag))
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    # Auto-detect dbt
    venv_dbt = Path(__file__).parent.parent / '01_data_generation' / '.venv' / 'bin' / 'dbt'
    ...
    for i, row in enumerate(eval_rows):
        msgs = row.get('messages') or [
            ...
            {"role": "user",   "content": f"Business question: {row['question']}\n\nSQL schemas:\n{row['context']}"},
        ]
        ...
        print(f"  [{i+1:3}/{len(eval_rows)}] {status}  files={r['files_generated']}  {row.get('question','')[:55]}")
    ...
    for name, value in metrics:
        bar = "█" * int(value * 20) if value <= 1 else ""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Baseline dbt DAG generation evaluation")
